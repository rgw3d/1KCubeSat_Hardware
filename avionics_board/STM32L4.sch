EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr B 17000 11000
encoding utf-8
Sheet 2 25
Title "STM32L4"
Date "2020-04-24"
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L MCU_ST_STM32L4:STM32L496ZGTx U1
U 1 1 5EA41511
P 8300 6125
F 0 "U1" H 8300 8900 50  0000 C CNN
F 1 "STM32L496ZGTx" H 8250 8700 138 0000 C CNN
F 2 "Package_QFP:LQFP-144_20x20mm_P0.5mm" H 7300 2725 50  0001 R CNN
F 3 "http://www.st.com/st-web-ui/static/active/en/resource/technical/document/datasheet/DM00284211.pdf" H 8300 6125 50  0001 C CNN
F 4 "STM32L496ZGT3" H 8300 6125 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/stmicroelectronics/STM32L496ZGT3/497-17177-ND/6709882" H 8300 6125 50  0001 C CNN "Order"
	1    8300 6125
	1    0    0    -1  
$EndComp
$Sheet
S 14875 8090 1600 985 
U 5EA7CBB1
F0 "MRAM" 50
F1 "MRAM.sch" 50
F2 "QUADSPI_CLK" O L 14875 8175 50 
F3 "QUADSPI_IO0" O L 14875 8325 50 
F4 "QUADSPI_IO1" O L 14875 8450 50 
F5 "QUADSPI_CS" O L 14875 8575 50 
F6 "VSS" O L 14875 9000 50 
F7 "VDD" O L 14875 8875 50 
$EndSheet
Text Label 6550 9025 2    50   ~ 0
QUADSPI_BK1_NCS
Text Label 10050 4525 0    50   ~ 0
QUADSPI_BK1_IO1
Wire Wire Line
	7100 7025 6550 7025
Wire Wire Line
	7100 7225 6550 7225
Text Label 6550 7025 2    50   ~ 0
QUADSPI_BK1_IO0
Text Label 6550 7225 2    50   ~ 0
QUADSPI_CLK
Wire Wire Line
	7100 9025 6550 9025
Wire Wire Line
	9500 4525 10050 4525
Text Label 14600 8175 2    50   ~ 0
QUADSPI_CLK
Wire Wire Line
	14600 8175 14875 8175
Wire Wire Line
	14875 8325 14600 8325
Wire Wire Line
	14600 8450 14875 8450
Wire Wire Line
	14875 8575 14600 8575
$Sheet
S 14875 6925 1600 975 
U 5EAB027D
F0 "SD_CARD" 50
F1 "SD_CARD.sch" 50
F2 "SD_DAT0" I L 14875 7000 50 
F3 "SD_DAT1" I L 14875 7100 50 
F4 "SD_DAT2" I L 14875 7200 50 
F5 "SD_DAT3" I L 14875 7300 50 
F6 "VDD" I L 14875 7675 50 
F7 "SD_CLK" I L 14875 7400 50 
F8 "SD_CMD" I L 14875 7500 50 
F9 "VSS" I L 14875 7775 50 
$EndSheet
Text Notes 16475 7525 2    138  ~ 0
SD CARD\n
$Sheet
S 4100 1650 1200 625 
U 5EAB0A1F
F0 "WATCHDOG" 50
F1 "WATCHDOG.sch" 50
F2 "VDD" O R 5300 2025 50 
F3 "VSS" O R 5300 2125 50 
F4 "RST" O R 5300 1725 50 
F5 "DONE" I R 5300 1825 50 
F6 "WAKE" O R 5300 1925 50 
$EndSheet
$Sheet
S 14875 4250 1600 700 
U 5EAB1057
F0 "GPS_INTERFACE" 50
F1 "GPS_INTERFACE.sch" 50
F2 "PWR" I L 14875 4625 50 
F3 "GND" I L 14875 4725 50 
F4 "UART_TX" I L 14875 4325 50 
F5 "UART_RX" O L 14875 4425 50 
F6 "RESET" I L 14875 4525 50 
$EndSheet
$Sheet
S 525  5025 1700 1500
U 5EAB17B1
F0 "SUNSENSOR_INTERFACE" 50
F1 "SUNSENSOR.sch" 50
F2 "PWR" I R 2225 6250 50 
F3 "GND" I R 2225 6350 50 
F4 "SCK" I R 2225 5125 50 
F5 "MISO" O R 2225 5225 50 
F6 "MOSI" I R 2225 5325 50 
F7 "FRAME_RDY_1" O R 2225 5425 50 
F8 "CS_1" I R 2225 5525 50 
F9 "FRAME_RDY_2" O R 2225 5625 50 
F10 "CS_2" I R 2225 5725 50 
F11 "FRAME_RDY_3" O R 2225 5825 50 
F12 "CS_3" I R 2225 5925 50 
$EndSheet
Text Notes 16475 6025 2    138  ~ 0
IRIDIUM
Text Notes 16025 4800 0    138  ~ 0
GPS
Text Notes 525  10250 0    138  ~ 0
DEBUG / \nPROGRAM
Text Notes 4100 2200 0    138  ~ 0
WATCH- \n DOG
Text Notes 550  6050 0    138  ~ 0
SUN \nSENSOR
Wire Wire Line
	9500 7025 10050 7025
Text Label 10050 7025 0    50   ~ 0
SDMMC1_DO
Wire Wire Line
	9500 7125 10050 7125
Text Label 10050 7125 0    50   ~ 0
SDMMC1_D1
Wire Wire Line
	9500 7225 10050 7225
Text Label 10050 7225 0    50   ~ 0
SDMMC1_D2
Wire Wire Line
	9500 7325 10050 7325
Text Label 10050 7325 0    50   ~ 0
SDMMC1_D3
Wire Wire Line
	9500 7425 10050 7425
Text Label 10050 7425 0    50   ~ 0
SDMMC1_CK
Wire Wire Line
	9500 8125 10050 8125
Text Label 10050 8125 0    50   ~ 0
SDMMC1_CMD
Text Label 14525 7000 2    50   ~ 0
SDMMC1_DO
Text Label 14525 7100 2    50   ~ 0
SDMMC1_D1
Text Label 14525 7200 2    50   ~ 0
SDMMC1_D2
Text Label 14525 7300 2    50   ~ 0
SDMMC1_D3
Text Label 14525 7400 2    50   ~ 0
SDMMC1_CK
Text Label 14525 7500 2    50   ~ 0
SDMMC1_CMD
Wire Wire Line
	14525 7000 14875 7000
Wire Wire Line
	14525 7100 14875 7100
Wire Wire Line
	14525 7200 14875 7200
Wire Wire Line
	14875 7300 14525 7300
Wire Wire Line
	14525 7400 14875 7400
Wire Wire Line
	14875 7500 14525 7500
Wire Wire Line
	7100 2825 6625 2825
Text Label 6625 2825 2    50   ~ 0
RST
Text Label 5775 1725 0    50   ~ 0
RST
$Sheet
S 525  6700 1700 1475
U 5EB482DD
F0 "ACS" 50
F1 "ACS.sch" 50
F2 "GND" O R 2225 8025 50 
F3 "PWR_COIL" I R 2225 7925 50 
F4 "PWR_LOGIC" I R 2225 7825 50 
F5 "X_nSLEEP" I R 2225 6800 50 
F6 "X_PH" I R 2225 6900 50 
F7 "X_EN" I R 2225 7000 50 
F8 "Y_nSLEEP" I R 2225 7100 50 
F9 "Y_PH" I R 2225 7200 50 
F10 "Y_EN" I R 2225 7300 50 
F11 "Z_nSLEEP" I R 2225 7400 50 
F12 "Z_PH" I R 2225 7500 50 
F13 "Z_EN" I R 2225 7600 50 
$EndSheet
$Sheet
S 525  8350 1700 1925
U 5EAB1235
F0 "DEBUG_INTERFACE" 50
F1 "DEBUG_INTERFACE.sch" 50
F2 "NRST" I R 2225 9025 50 
F3 "TARGET_GND" I R 2225 10200 50 
F4 "TARGET_VDD" I R 2225 10100 50 
F5 "JTAG_TRST" I R 2225 9125 50 
F6 "JTAG_TCK_SWD_SWCLK" I R 2225 9225 50 
F7 "JTAG_TDO_SWD_SWO" I R 2225 9325 50 
F8 "JTAG_TMS_SWD_SWDIO" I R 2225 9425 50 
F9 "JTAG_TDI" I R 2225 9525 50 
F10 "TARGET_UART_RX" I R 2225 9725 50 
F11 "TARGET_UART_TX" I R 2225 9825 50 
F12 "LED_GPIO1" I R 2225 8425 50 
F13 "LED_GPIO2" I R 2225 8525 50 
F14 "LED_GPIO3" I R 2225 8625 50 
F15 "LED_GPIO4" I R 2225 8725 50 
F16 "LED_GPIO5" I R 2225 8825 50 
$EndSheet
Text Notes 550  8150 0    138  ~ 0
ACS\nATTITUDE \nCONTROL \nSUBSYSTEM
Text Notes 16025 5425 0    50   ~ 0
3.3V Logic
Text Notes 625  8525 0    50   ~ 0
Input voltage to STM\n
Text Notes 550  7150 0    50   ~ 0
Regulate coil voltage \nfor repeatable magnetic \nperformance
Text Notes 16450 4425 2    50   ~ 0
3.3V Power\n200mA
Text Notes 16450 8275 2    50   ~ 0
3.0V-3.6V\n40mA peak, budget 80mA
Text Notes 550  5200 0    50   ~ 0
3.0 to 3.6V\n7mA per sensor
Text Notes 4800 1750 2    50   ~ 0
VDD: 1.8V to 5.5V
Text Notes 15775 7100 0    50   ~ 0
VDD: 2.7V to 3.6V \n250mA peak
Text Notes 550  6875 0    50   ~ 0
COIL: 0-11V, 1A at least\nLogic: 1.8-7V, 20mA
Wire Wire Line
	9500 4325 10050 4325
Wire Wire Line
	9500 4225 10050 4225
Wire Wire Line
	9500 4125 10050 4125
Wire Wire Line
	9500 3825 10050 3825
Text Label 10050 3825 0    50   ~ 0
USART1_RX
Text Label 10050 3925 0    50   ~ 0
CAN1_RX
Text Label 10050 4025 0    50   ~ 0
CAN1_TX
Text Label 10050 4125 0    50   ~ 0
JTMS_SWDIO
Text Label 10050 4225 0    50   ~ 0
JTCK_SWCLK
Text Label 10050 4325 0    50   ~ 0
JTDI
Wire Wire Line
	9500 3025 10050 3025
Wire Wire Line
	9500 3125 10050 3125
Wire Wire Line
	9500 3225 10050 3225
Wire Wire Line
	9500 3325 10050 3325
Wire Wire Line
	9500 3425 10050 3425
Wire Wire Line
	9500 3525 10050 3525
Wire Wire Line
	9500 3725 10050 3725
Text Label 10050 2825 0    50   ~ 0
UART4_TX
Text Label 10050 2925 0    50   ~ 0
UART4_RX
Text Label 10050 3025 0    50   ~ 0
ADC1_IN7
Text Label 10050 3125 0    50   ~ 0
ADC1_IN8
Text Label 10050 3225 0    50   ~ 0
ADC1_IN9
Text Label 10050 3325 0    50   ~ 0
ADC1_IN10
Text Label 10050 3425 0    50   ~ 0
ADC1_IN11
Text Label 10050 3525 0    50   ~ 0
ADC1_IN12
Text Label 10050 3725 0    50   ~ 0
USART1_TX
Text Label 10050 4625 0    50   ~ 0
USART3_RTS
Wire Wire Line
	9500 4625 10050 4625
Wire Wire Line
	9500 4725 10050 4725
Wire Wire Line
	9500 4825 10050 4825
Wire Wire Line
	9500 4925 10050 4925
Wire Wire Line
	9500 5025 10050 5025
Wire Wire Line
	9500 5125 10050 5125
Wire Wire Line
	9500 5225 10050 5225
Wire Wire Line
	9500 5325 10050 5325
Wire Wire Line
	9500 5425 10050 5425
Wire Wire Line
	9500 5525 10050 5525
Wire Wire Line
	9500 5625 10050 5625
Wire Wire Line
	9500 5725 10050 5725
Wire Wire Line
	9500 5825 10050 5825
Wire Wire Line
	9500 6625 10050 6625
Wire Wire Line
	9500 6725 10050 6725
Wire Wire Line
	9500 6825 10050 6825
Wire Wire Line
	9500 8225 10050 8225
Wire Wire Line
	9500 8325 10050 8325
Wire Wire Line
	9500 8425 10050 8425
Wire Wire Line
	9500 8525 10050 8525
Wire Wire Line
	9500 8625 10050 8625
Wire Wire Line
	9500 8725 10050 8725
Wire Wire Line
	9500 8825 10050 8825
Wire Wire Line
	9500 8925 10050 8925
Wire Wire Line
	9500 9025 10050 9025
Wire Wire Line
	9500 9125 10050 9125
Wire Wire Line
	9500 9225 10050 9225
Wire Wire Line
	9500 9325 10050 9325
Wire Wire Line
	6550 8725 7100 8725
Wire Wire Line
	6550 8825 7100 8825
Wire Wire Line
	6550 8925 7100 8925
Wire Wire Line
	6550 8525 7100 8525
Wire Wire Line
	6550 8625 7100 8625
Wire Wire Line
	6550 8125 7100 8125
Wire Wire Line
	6550 8225 7100 8225
Wire Wire Line
	6550 8325 7100 8325
Wire Wire Line
	6550 7525 7100 7525
Wire Wire Line
	6550 6825 7100 6825
Wire Wire Line
	6550 6925 7100 6925
Wire Wire Line
	6550 4225 7100 4225
Wire Wire Line
	6550 4325 7100 4325
Wire Wire Line
	6550 5225 7100 5225
Wire Wire Line
	6550 5325 7100 5325
Wire Wire Line
	6550 5425 7100 5425
Wire Wire Line
	6550 5525 7100 5525
Wire Wire Line
	6550 5625 7100 5625
Wire Wire Line
	6550 5725 7100 5725
Wire Wire Line
	6550 5825 7100 5825
Wire Wire Line
	6550 5925 7100 5925
Text Label 10050 4825 0    50   ~ 0
JTDO_TRACESWO
Text Label 10050 4925 0    50   ~ 0
NJRST
NoConn ~ 10050 5025
NoConn ~ 10050 5125
NoConn ~ 10050 5225
NoConn ~ 10050 4725
Text Label 10050 5325 0    50   ~ 0
TIM16_CH1
Text Label 10050 5425 0    50   ~ 0
TIM17_CH1
Text Label 10050 5525 0    50   ~ 0
LPUART1_RX
Text Label 10050 5625 0    50   ~ 0
LPUART1_TX
NoConn ~ 10050 5725
Text Label 10050 5825 0    50   ~ 0
SPI2_SCK
Wire Wire Line
	9500 5925 10050 5925
Text Label 10050 5925 0    50   ~ 0
SPI2_MISO
Wire Wire Line
	9500 6025 10050 6025
Text Label 10050 6025 0    50   ~ 0
SPI2_MOSI
Wire Wire Line
	9500 6225 10050 6225
Wire Wire Line
	9500 6325 10050 6325
Wire Wire Line
	9500 6425 10050 6425
Wire Wire Line
	9500 6525 10050 6525
Wire Wire Line
	9500 6925 10050 6925
Text Label 10050 6225 0    50   ~ 0
ADC1_IN1
Text Label 10050 6325 0    50   ~ 0
ADC1_IN2
Text Label 10050 6425 0    50   ~ 0
ADC1_IN3
Text Label 10050 6525 0    50   ~ 0
ADC1_IN4
Text Label 10050 6625 0    50   ~ 0
USART3_TX
Text Label 10050 6725 0    50   ~ 0
USART3_RX
NoConn ~ 10050 6825
NoConn ~ 10050 6925
Wire Wire Line
	9500 7525 10050 7525
Wire Wire Line
	9500 7725 10050 7725
NoConn ~ 10050 7725
Wire Wire Line
	9500 7925 10050 7925
NoConn ~ 10050 7925
Wire Wire Line
	9500 8025 10050 8025
NoConn ~ 10050 8025
Text Label 10050 9025 0    50   ~ 0
USART3_CTS
Wire Wire Line
	9500 9425 10050 9425
NoConn ~ 10050 9425
NoConn ~ 10050 9325
NoConn ~ 10050 9225
NoConn ~ 10050 9125
NoConn ~ 10050 8925
NoConn ~ 10050 8825
Text Label 10050 8525 0    50   ~ 0
USART2_RX
Text Label 10050 8425 0    50   ~ 0
USART2_TX
NoConn ~ 10050 8325
NoConn ~ 10050 8225
Wire Wire Line
	6550 7925 7100 7925
Wire Wire Line
	6550 8025 7100 8025
Wire Wire Line
	6550 9125 7100 9125
Wire Wire Line
	6550 7625 7100 7625
Wire Wire Line
	6550 7725 7100 7725
Text Label 6550 7625 2    50   ~ 0
I2C4_SCL
Text Label 6550 7725 2    50   ~ 0
I2C4_SDA
Wire Wire Line
	6550 7125 7100 7125
Text Label 6550 7125 2    50   ~ 0
TIM15_CH1
Wire Wire Line
	6550 7325 7100 7325
Wire Wire Line
	6550 7425 7100 7425
Wire Wire Line
	6550 9225 7100 9225
Wire Wire Line
	6550 9325 7100 9325
Wire Wire Line
	6550 9425 7100 9425
Text Label 6550 9225 2    50   ~ 0
SPI1_SCK
Text Label 6550 9325 2    50   ~ 0
SPI1_MISO
Text Label 6550 9425 2    50   ~ 0
SPI1_MOSI
Wire Wire Line
	6550 6225 7100 6225
Wire Wire Line
	6550 6325 7100 6325
Text Label 6550 6225 2    50   ~ 0
I2C2_SDA
Text Label 6550 6325 2    50   ~ 0
I2C2_SCL
Wire Wire Line
	6550 6425 7100 6425
Wire Wire Line
	7100 6525 6550 6525
Wire Wire Line
	6550 6625 7100 6625
Wire Wire Line
	7100 6725 6550 6725
Text Label 14600 8325 2    50   ~ 0
QUADSPI_BK1_IO0
Text Label 14600 8575 2    50   ~ 0
QUADSPI_BK1_NCS
Text Label 14600 8450 2    50   ~ 0
QUADSPI_BK1_IO1
Text Label 6550 5225 2    50   ~ 0
I2C3_SCL
Text Label 6550 5325 2    50   ~ 0
I2C3_SDA
Text Label 6550 5425 2    50   ~ 0
SPI3_SCK
Text Label 6550 5525 2    50   ~ 0
SPI3_MISO
Text Label 6550 5625 2    50   ~ 0
SPI3_MOSI
Text Label 6550 5825 2    50   ~ 0
I2C1_SDA
Text Label 6550 5925 2    50   ~ 0
I2C1_SCL
NoConn ~ 6550 4225
NoConn ~ 6550 4325
Text Notes 10500 3475 0    50   ~ 0
11 & 12 are currently unused\n
Wire Wire Line
	2225 9525 2775 9525
Wire Wire Line
	2225 9225 2775 9225
Wire Wire Line
	2225 9425 2775 9425
Text Label 2775 9425 0    50   ~ 0
JTMS_SWDIO
Text Label 2775 9225 0    50   ~ 0
JTCK_SWCLK
Text Label 2775 9525 0    50   ~ 0
JTDI
Wire Wire Line
	2225 9325 2775 9325
Wire Wire Line
	2225 9125 2775 9125
Text Label 2775 9325 0    50   ~ 0
JTDO_TRACESWO
Text Label 2775 9125 0    50   ~ 0
NJRST
Wire Wire Line
	2775 9025 2225 9025
Text Label 2775 9025 0    50   ~ 0
RST
$Comp
L power:GND #PWR011
U 1 1 5ED8A29B
P 7800 9775
F 0 "#PWR011" H 7800 9525 50  0001 C CNN
F 1 "GND" H 7805 9602 50  0000 C CNN
F 2 "" H 7800 9775 50  0001 C CNN
F 3 "" H 7800 9775 50  0001 C CNN
	1    7800 9775
	1    0    0    -1  
$EndComp
Wire Wire Line
	7800 9775 7900 9775
Wire Wire Line
	7900 9775 7900 9725
Wire Wire Line
	7800 9775 7800 9725
Wire Wire Line
	7900 9775 8000 9775
Wire Wire Line
	8000 9775 8000 9725
Connection ~ 7900 9775
Wire Wire Line
	8000 9775 8100 9775
Wire Wire Line
	8100 9775 8100 9725
Connection ~ 8000 9775
Wire Wire Line
	8100 9775 8200 9775
Wire Wire Line
	8200 9775 8200 9725
Connection ~ 8100 9775
Wire Wire Line
	8200 9775 8300 9775
Wire Wire Line
	8300 9775 8300 9725
Connection ~ 8200 9775
Wire Wire Line
	8300 9775 8400 9775
Wire Wire Line
	8400 9775 8400 9725
Connection ~ 8300 9775
Wire Wire Line
	8400 9775 8500 9775
Wire Wire Line
	8500 9775 8500 9725
Connection ~ 8400 9775
Wire Wire Line
	8700 9775 8700 9725
Wire Wire Line
	8500 9775 8600 9775
Connection ~ 8500 9775
Wire Wire Line
	8600 9725 8600 9775
Connection ~ 8600 9775
Wire Wire Line
	8600 9775 8700 9775
Wire Wire Line
	8800 9725 8800 9775
Wire Wire Line
	8800 9775 8700 9775
Connection ~ 8700 9775
Wire Wire Line
	8800 9775 8900 9775
Wire Wire Line
	8900 9775 8900 9725
Connection ~ 8800 9775
$Comp
L power:GND #PWR08
U 1 1 5EE46BFF
P 14500 6550
F 0 "#PWR08" H 14500 6300 50  0001 C CNN
F 1 "GND" H 14505 6377 50  0000 C CNN
F 2 "" H 14500 6550 50  0001 C CNN
F 3 "" H 14500 6550 50  0001 C CNN
	1    14500 6550
	-1   0    0    -1  
$EndComp
Wire Wire Line
	14875 6650 14700 6650
$Comp
L power:GND #PWR013
U 1 1 5EE63593
P 14700 4725
F 0 "#PWR013" H 14700 4475 50  0001 C CNN
F 1 "GND" H 14705 4552 50  0000 C CNN
F 2 "" H 14700 4725 50  0001 C CNN
F 3 "" H 14700 4725 50  0001 C CNN
	1    14700 4725
	-1   0    0    -1  
$EndComp
Wire Wire Line
	14875 4725 14700 4725
$Comp
L power:GND #PWR04
U 1 1 5EE71E3E
P 2550 8025
F 0 "#PWR04" H 2550 7775 50  0001 C CNN
F 1 "GND" H 2555 7852 50  0000 C CNN
F 2 "" H 2550 8025 50  0001 C CNN
F 3 "" H 2550 8025 50  0001 C CNN
	1    2550 8025
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR03
U 1 1 5EE80B0A
P 2475 6350
F 0 "#PWR03" H 2475 6100 50  0001 C CNN
F 1 "GND" H 2480 6177 50  0000 C CNN
F 2 "" H 2475 6350 50  0001 C CNN
F 3 "" H 2475 6350 50  0001 C CNN
	1    2475 6350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR017
U 1 1 5EEBE5DE
P 5475 2125
F 0 "#PWR017" H 5475 1875 50  0001 C CNN
F 1 "GND" H 5480 1952 50  0000 C CNN
F 2 "" H 5475 2125 50  0001 C CNN
F 3 "" H 5475 2125 50  0001 C CNN
	1    5475 2125
	1    0    0    -1  
$EndComp
Wire Wire Line
	5300 2125 5475 2125
Wire Wire Line
	2225 7300 2775 7300
Wire Wire Line
	2225 7600 2775 7600
Text Label 2775 7300 0    50   ~ 0
TIM16_CH1
Text Label 2775 7600 0    50   ~ 0
TIM17_CH1
Wire Wire Line
	2775 7000 2225 7000
Text Label 2775 7000 0    50   ~ 0
TIM15_CH1
Wire Wire Line
	2225 6900 2775 6900
Wire Wire Line
	2225 7200 2775 7200
Wire Wire Line
	2225 7500 2775 7500
Wire Wire Line
	2225 6800 2775 6800
Wire Wire Line
	2225 7100 2775 7100
Wire Wire Line
	2225 7400 2775 7400
Text Label 14325 5500 2    50   ~ 0
USART3_RTS
Wire Wire Line
	14875 5500 14325 5500
Wire Wire Line
	14875 5200 14325 5200
Wire Wire Line
	14875 5300 14325 5300
Text Label 14325 5200 2    50   ~ 0
USART3_TX
Text Label 14325 5300 2    50   ~ 0
USART3_RX
Wire Wire Line
	14875 5400 14325 5400
Text Label 14325 5400 2    50   ~ 0
USART3_CTS
Wire Wire Line
	14875 6550 14700 6550
Wire Wire Line
	14700 6550 14700 6650
Wire Wire Line
	14875 4325 14325 4325
Wire Wire Line
	14875 4425 14325 4425
Text Label 14325 4425 2    50   ~ 0
USART2_RX
Text Label 14325 4325 2    50   ~ 0
USART2_TX
Wire Wire Line
	2225 9725 2775 9725
Wire Wire Line
	2225 9825 2775 9825
Text Label 2775 9725 0    50   ~ 0
LPUART1_RX
Text Label 2775 9825 0    50   ~ 0
LPUART1_TX
Wire Wire Line
	2225 5125 2775 5125
Wire Wire Line
	2225 5325 2775 5325
Text Label 2775 5225 0    50   ~ 0
SPI3_MISO
Text Label 2775 5325 0    50   ~ 0
SPI3_MOSI
Wire Wire Line
	2225 5225 2775 5225
Text Label 2775 5125 0    50   ~ 0
SPI3_SCK
Wire Wire Line
	6550 4525 7100 4525
Wire Wire Line
	6550 4625 7100 4625
Wire Wire Line
	6550 4725 7100 4725
Wire Wire Line
	6550 4825 7100 4825
Wire Wire Line
	6550 4925 7100 4925
Wire Wire Line
	6550 5025 7100 5025
Wire Wire Line
	6550 5125 7100 5125
Text Label 6550 4525 2    50   ~ 0
MAG_INT
Text Label 6550 4625 2    50   ~ 0
WATCHDOG_WAKE
Text Label 6550 4725 2    50   ~ 0
GYRO_INT_1
Text Label 6550 4825 2    50   ~ 0
GYRO_INT_2
Text Label 6550 4925 2    50   ~ 0
IRIDIUM_RING
Text Label 6550 5025 2    50   ~ 0
IRIDIUM_NET_AVAIL
Text Label 6550 5125 2    50   ~ 0
SUN_FRAME_RDY_1
Text Label 10050 8625 0    50   ~ 0
SUN_FRAME_RDY_2
Text Label 10050 8725 0    50   ~ 0
SUN_FRAME_RDY_3
Wire Wire Line
	2225 5625 2775 5625
Wire Wire Line
	2225 5825 2775 5825
Text Label 2775 5625 0    50   ~ 0
SUN_FRAME_RDY_2
Text Label 2775 5825 0    50   ~ 0
SUN_FRAME_RDY_3
Wire Wire Line
	2225 5425 2775 5425
Text Label 2775 5425 0    50   ~ 0
SUN_FRAME_RDY_1
Wire Wire Line
	5775 1925 5300 1925
Text Label 5775 1925 0    50   ~ 0
WATCHDOG_WAKE
Wire Wire Line
	14325 5975 14875 5975
Text Label 14325 5975 2    50   ~ 0
IRIDIUM_RING
Wire Wire Line
	14325 6075 14875 6075
Text Label 14325 6075 2    50   ~ 0
IRIDIUM_NET_AVAIL
Text Label 6550 7925 2    50   ~ 0
WATCHDOG_DONE
Text Label 5775 1825 0    50   ~ 0
WATCHDOG_DONE
Wire Wire Line
	5775 1825 5300 1825
Text Label 6550 8025 2    50   ~ 0
MAG_CS
Text Label 6550 8125 2    50   ~ 0
GYRO_CS
Wire Wire Line
	6550 8425 7100 8425
Text Label 6550 8225 2    50   ~ 0
ACS_X_SLEEP
Text Label 6550 8325 2    50   ~ 0
ACS_X_PH
Text Label 6550 8425 2    50   ~ 0
ACS_Y_SLEEP
Text Label 6550 8525 2    50   ~ 0
ACS_Y_PH
Text Label 6550 8625 2    50   ~ 0
ACS_Z_SLEEP
Text Label 6550 8725 2    50   ~ 0
ACS_Z_PH
Text Label 2775 6800 0    50   ~ 0
ACS_X_SLEEP
Text Label 2775 6900 0    50   ~ 0
ACS_X_PH
Text Label 2775 7100 0    50   ~ 0
ACS_Y_SLEEP
Text Label 2775 7200 0    50   ~ 0
ACS_Y_PH
Text Label 2775 7400 0    50   ~ 0
ACS_Z_SLEEP
Text Label 2775 7500 0    50   ~ 0
ACS_Z_PH
Text Label 6550 8825 2    50   ~ 0
SUN_CS_1
Text Label 6550 8925 2    50   ~ 0
SUN_CS_2
Text Label 6550 9125 2    50   ~ 0
SUN_CS_3
Text Label 2775 5525 0    50   ~ 0
SUN_CS_1
Text Label 2775 5725 0    50   ~ 0
SUN_CS_2
Wire Wire Line
	2225 5525 2775 5525
Wire Wire Line
	2225 5725 2775 5725
Wire Wire Line
	2225 5925 2775 5925
Text Label 2775 5925 0    50   ~ 0
SUN_CS_3
Text Label 6550 6425 2    50   ~ 0
LED_1
Text Label 6550 6525 2    50   ~ 0
LED_2
Text Label 6550 6625 2    50   ~ 0
LED_3
Text Label 6550 6725 2    50   ~ 0
LED_4
Text Label 6550 6825 2    50   ~ 0
LED_5
Text Label 2775 8425 0    50   ~ 0
LED_1
Text Label 2775 8525 0    50   ~ 0
LED_2
Text Label 2775 8625 0    50   ~ 0
LED_3
Text Label 2775 8725 0    50   ~ 0
LED_4
Text Label 2775 8825 0    50   ~ 0
LED_5
Wire Wire Line
	2225 8625 2775 8625
Wire Wire Line
	2225 8825 2775 8825
Wire Wire Line
	2225 8725 2775 8725
Wire Wire Line
	2225 8525 2775 8525
Wire Wire Line
	2775 8425 2225 8425
$Comp
L power:GND #PWR06
U 1 1 5FAAD075
P 2475 10200
F 0 "#PWR06" H 2475 9950 50  0001 C CNN
F 1 "GND" H 2480 10027 50  0000 C CNN
F 2 "" H 2475 10200 50  0001 C CNN
F 3 "" H 2475 10200 50  0001 C CNN
	1    2475 10200
	1    0    0    -1  
$EndComp
Text Label 6550 6925 2    50   ~ 0
IRIDIUM_DTR
Text Label 6550 7325 2    50   ~ 0
IRIDIUM_DCD
Text Label 14325 5675 2    50   ~ 0
IRIDIUM_DTR
Wire Wire Line
	14325 5675 14875 5675
Wire Wire Line
	14325 5775 14875 5775
Wire Wire Line
	14325 6175 14875 6175
Wire Wire Line
	14325 5875 14875 5875
Wire Wire Line
	14325 6450 14875 6450
Text Label 14325 5775 2    50   ~ 0
IRIDIUM_DCD
Text Label 6550 7425 2    50   ~ 0
IRIDIUM_DSR
Text Label 6550 7525 2    50   ~ 0
IRIDIUM_ON_OFF
Text Label 14325 5875 2    50   ~ 0
IRIDIUM_DSR
Text Label 14325 6175 2    50   ~ 0
IRIDIUM_ON_OFF
Text Label 6550 5725 2    50   ~ 0
GPS_RESET
Text Label 14325 4525 2    50   ~ 0
GPS_RESET
Wire Wire Line
	14325 4525 14875 4525
$Sheet
S 11825 7400 925  600 
U 5FEDB823
F0 "OSC32_TCXO" 50
F1 "OSC32_TCXO.sch" 50
F2 "PWR" I L 11825 7725 50 
F3 "GND" I L 11825 7825 50 
F4 "OE" I L 11825 7525 50 
F5 "OUT" I L 11825 7625 50 
$EndSheet
Text Notes 10675 7675 0    50   ~ 0
Leave OSC32_OUT as NC \nwhen using TCXO\n\n
Wire Wire Line
	9500 7625 11825 7625
NoConn ~ 10050 7525
$Comp
L power:GND #PWR015
U 1 1 5FFDA728
P 11625 8075
F 0 "#PWR015" H 11625 7825 50  0001 C CNN
F 1 "GND" H 11630 7902 50  0000 C CNN
F 2 "" H 11625 8075 50  0001 C CNN
F 3 "" H 11625 8075 50  0001 C CNN
	1    11625 8075
	1    0    0    -1  
$EndComp
Wire Wire Line
	11625 7825 11625 8075
Wire Wire Line
	11625 7825 11825 7825
$Sheet
S 4100 3975 925  625 
U 600564FD
F0 "OSC_HSE_TCXO" 50
F1 "OSC_HSE_TCXO.sch" 50
F2 "PWR" I R 5025 4450 50 
F3 "GND" I R 5025 4550 50 
F4 "OE" I R 5025 4225 50 
F5 "OUT" I R 5025 4125 50 
$EndSheet
Wire Wire Line
	5025 4125 7100 4125
$Comp
L power:GND #PWR07
U 1 1 600F8A9C
P 5225 4800
F 0 "#PWR07" H 5225 4550 50  0001 C CNN
F 1 "GND" H 5230 4627 50  0000 C CNN
F 2 "" H 5225 4800 50  0001 C CNN
F 3 "" H 5225 4800 50  0001 C CNN
	1    5225 4800
	1    0    0    -1  
$EndComp
Wire Wire Line
	5225 4550 5225 4800
Wire Wire Line
	5025 4550 5225 4550
Wire Wire Line
	5025 4225 5725 4225
Wire Wire Line
	5725 4225 5725 6025
Wire Wire Line
	5725 6025 7100 6025
Text Notes 5050 3950 0    50   ~ 0
Leave RCC_OSC_OUT as NC when using TCXO\n
Text Notes 4125 4075 0    50   ~ 0
3.3V Power
Text Notes 12325 7500 0    50   ~ 0
1.5V-5.5V 
Wire Wire Line
	9500 4025 11450 4025
Wire Wire Line
	9500 3925 11450 3925
$Comp
L power:GND #PWR016
U 1 1 6034957C
P 11175 4275
F 0 "#PWR016" H 11175 4025 50  0001 C CNN
F 1 "GND" H 11180 4102 50  0000 C CNN
F 2 "" H 11175 4275 50  0001 C CNN
F 3 "" H 11175 4275 50  0001 C CNN
	1    11175 4275
	1    0    0    -1  
$EndComp
Text Notes 12850 8000 2    138  ~ 0
32kHz \nTCXO 
Text Notes 4125 4550 0    138  ~ 0
10MHz \nTCXO 
Text Notes 5525 1500 0    50   ~ 0
VDD & VDDIO2:\nUse one 0.1uF cap per power pin.\nUse one 4.7uF cap per power pin group.\n\nVDDA: \nUse one 0.01uF & one 1uF\n\nVREF: \nUse one 0.1uF & one 1uF
Text Notes 11625 4225 0    138  ~ 0
CAN \n  TRANS-\n    CEIVER
Wire Wire Line
	7800 2350 7900 2350
Wire Wire Line
	8600 2350 8600 2450
Wire Wire Line
	8500 2525 8500 2350
Connection ~ 8500 2350
Wire Wire Line
	8500 2350 8600 2350
Wire Wire Line
	8400 2525 8400 2350
Connection ~ 8400 2350
Wire Wire Line
	8400 2350 8500 2350
Wire Wire Line
	8300 2525 8300 2350
Connection ~ 8300 2350
Wire Wire Line
	8300 2350 8400 2350
Wire Wire Line
	8200 2525 8200 2350
Connection ~ 8200 2350
Wire Wire Line
	8200 2350 8300 2350
Wire Wire Line
	8100 2525 8100 2350
Connection ~ 8100 2350
Wire Wire Line
	8100 2350 8200 2350
Wire Wire Line
	8000 2525 8000 2350
Connection ~ 8000 2350
Wire Wire Line
	8000 2350 8100 2350
Wire Wire Line
	7900 2525 7900 2350
Connection ~ 7900 2350
Wire Wire Line
	7900 2350 8000 2350
$Comp
L Device:C_Small C6
U 1 1 607DF25E
P 7525 850
F 0 "C6" V 7425 925 50  0000 C CNN
F 1 "0.1uF" V 7400 725 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 850 50  0001 C CNN
F 3 "~" H 7525 850 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 850 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 850 50  0001 C CNN "Order"
F 6 "25V" V 7475 700 50  0000 C CNN "Voltage"
	1    7525 850 
	0    1    1    0   
$EndComp
Connection ~ 7800 2350
$Comp
L power:GND #PWR010
U 1 1 6098201A
P 7200 2200
F 0 "#PWR010" H 7200 1950 50  0001 C CNN
F 1 "GND" H 7325 2125 50  0000 C CNN
F 2 "" H 7200 2200 50  0001 C CNN
F 3 "" H 7200 2200 50  0001 C CNN
	1    7200 2200
	1    0    0    -1  
$EndComp
Wire Wire Line
	7200 2200 7200 1975
Wire Wire Line
	7200 1975 7425 1975
Connection ~ 7200 2200
Wire Wire Line
	7200 1975 7200 1750
Wire Wire Line
	7200 1750 7425 1750
Connection ~ 7200 1975
Wire Wire Line
	7200 1750 7200 1525
Wire Wire Line
	7200 1525 7425 1525
Connection ~ 7200 1750
Wire Wire Line
	7200 1300 7425 1300
Connection ~ 7200 1300
Wire Wire Line
	7200 1300 7200 1525
Wire Wire Line
	7425 1075 7200 1075
Connection ~ 7200 1075
Wire Wire Line
	7200 1075 7200 1300
Wire Wire Line
	7200 850  7425 850 
Wire Wire Line
	7200 850  7200 1075
Wire Wire Line
	9325 1750 9450 1750
Wire Wire Line
	9450 1975 9325 1975
Wire Wire Line
	9325 2200 9450 2200
Wire Wire Line
	9450 1750 9450 1975
Wire Wire Line
	9450 1975 9450 2200
Connection ~ 9450 1975
Wire Wire Line
	8375 1975 8150 1975
Wire Wire Line
	8150 1975 8150 2200
Wire Wire Line
	8150 2200 8375 2200
Wire Wire Line
	8575 1975 8700 1975
Wire Wire Line
	8700 1975 8700 2200
Wire Wire Line
	8700 2200 8575 2200
Wire Wire Line
	8800 2350 8800 2525
Wire Wire Line
	5775 3075 5775 3025
Wire Wire Line
	5775 3025 6125 3025
Wire Wire Line
	6125 3025 6125 3075
Wire Wire Line
	5775 3275 5775 3375
Wire Wire Line
	5775 3375 5950 3375
Wire Wire Line
	6125 3375 6125 3275
$Comp
L power:GND #PWR09
U 1 1 610595BA
P 5950 3450
F 0 "#PWR09" H 5950 3200 50  0001 C CNN
F 1 "GND" H 5955 3277 50  0000 C CNN
F 2 "" H 5950 3450 50  0001 C CNN
F 3 "" H 5950 3450 50  0001 C CNN
	1    5950 3450
	1    0    0    -1  
$EndComp
Wire Wire Line
	5950 3450 5950 3375
Connection ~ 5950 3375
Wire Wire Line
	5950 3375 6125 3375
Wire Wire Line
	6125 3025 7100 3025
Connection ~ 6125 3025
Wire Wire Line
	6125 3375 6900 3375
Wire Wire Line
	6900 3375 6900 3125
Wire Wire Line
	6900 3125 7100 3125
Connection ~ 6125 3375
Wire Wire Line
	8900 1975 8900 2200
Wire Wire Line
	8900 1750 8900 1975
Connection ~ 8900 1975
Wire Wire Line
	8900 1975 9125 1975
Wire Wire Line
	9125 2200 8900 2200
Wire Wire Line
	9125 1750 8900 1750
Wire Wire Line
	8600 2450 9000 2450
Wire Wire Line
	9000 2450 9000 2525
Connection ~ 8600 2450
Wire Wire Line
	8600 2450 8600 2525
$Comp
L power:GND #PWR012
U 1 1 6124C26D
P 8150 2200
F 0 "#PWR012" H 8150 1950 50  0001 C CNN
F 1 "GND" H 8275 2125 50  0000 C CNN
F 2 "" H 8150 2200 50  0001 C CNN
F 3 "" H 8150 2200 50  0001 C CNN
	1    8150 2200
	1    0    0    -1  
$EndComp
Connection ~ 8150 2200
Wire Wire Line
	8900 2200 8900 2350
Connection ~ 8900 2200
Wire Wire Line
	8700 1975 8700 1225
Wire Wire Line
	8700 1225 9600 1225
Connection ~ 8700 1975
Wire Wire Line
	8900 1550 9600 1550
Connection ~ 8900 1750
$Comp
L power:GND #PWR014
U 1 1 6156899A
P 9450 2200
F 0 "#PWR014" H 9450 1950 50  0001 C CNN
F 1 "GND" H 9575 2125 50  0000 C CNN
F 2 "" H 9450 2200 50  0001 C CNN
F 3 "" H 9450 2200 50  0001 C CNN
	1    9450 2200
	1    0    0    -1  
$EndComp
Connection ~ 9450 2200
$Comp
L Device:R_Small R1
U 1 1 6177536D
P 7300 2475
F 0 "R1" V 7375 2525 50  0000 C CNN
F 1 "0" V 7375 2400 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7300 2475 50  0001 C CNN
F 3 "~" H 7300 2475 50  0001 C CNN
F 4 "RK73Z1JTTD" V 7300 2475 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/koa-speer-electronics-inc/RK73Z1JTTD/2019-RK73Z1JTTDCT-ND/9847522" V 7300 2475 50  0001 C CNN "Order"
	1    7300 2475
	0    1    1    0   
$EndComp
Wire Wire Line
	7800 2350 7800 2525
Wire Wire Line
	7400 2475 7700 2475
Wire Wire Line
	7700 2475 7700 2525
Wire Wire Line
	7200 2475 7200 2350
Text Label 11075 1225 0    50   ~ 0
3.3V_STM_CORE
Text Label 5100 4450 0    50   ~ 0
3.3V_STM_CORE
Wire Wire Line
	5025 4450 5100 4450
Text Label 11625 7725 2    50   ~ 0
3.3V_STM_CORE
Wire Wire Line
	11625 7725 11825 7725
Text Label 5775 2025 0    50   ~ 0
3.3V_STM_CORE
Wire Wire Line
	5775 2025 5300 2025
Text Label 2775 10100 0    50   ~ 0
3.3V_STM_CORE
Wire Wire Line
	2775 10100 2225 10100
Text Label 2775 6250 0    50   ~ 0
3.3V_SUN_SENSOR
Wire Wire Line
	2775 6250 2225 6250
Text Label 2775 7825 0    50   ~ 0
3.3V_ACS_LOGIC
Wire Wire Line
	2775 7825 2225 7825
Text Label 2775 7925 0    50   ~ 0
5V_ACS_COIL
Wire Wire Line
	2775 7925 2225 7925
Text Label 14325 6450 2    50   ~ 0
5V_IRIDIUM
Text Label 14325 4625 2    50   ~ 0
3.3V_GPS
Wire Wire Line
	14325 4625 14875 4625
Text Label 14525 7675 2    50   ~ 0
3.3V_SD_CARD
Wire Wire Line
	14525 7675 14875 7675
Text Label 14600 8875 2    50   ~ 0
3.3V_MRAM
Wire Wire Line
	14600 8875 14875 8875
Text Notes 9900 1525 0    79   ~ 0
VDDIO2
Text Notes 9900 1225 0    79   ~ 0
VDDA\n
Wire Wire Line
	9900 1225 10400 1225
Wire Wire Line
	9900 1550 10400 1550
Connection ~ 10400 1225
Wire Wire Line
	10400 1225 11075 1225
$Comp
L power:GND #PWR018
U 1 1 61C481E5
P 14500 3725
F 0 "#PWR018" H 14500 3475 50  0001 C CNN
F 1 "GND" H 14505 3552 50  0000 C CNN
F 2 "" H 14500 3725 50  0001 C CNN
F 3 "" H 14500 3725 50  0001 C CNN
	1    14500 3725
	1    0    0    -1  
$EndComp
Wire Wire Line
	14500 3675 14500 3725
Text Label 2775 3900 0    50   ~ 0
GYRO_CS
Wire Wire Line
	2225 3900 2775 3900
Text Label 2775 3775 0    50   ~ 0
GYRO_INT_2
Wire Wire Line
	2225 3775 2775 3775
Wire Wire Line
	2225 3650 2775 3650
Text Label 2775 3650 0    50   ~ 0
GYRO_INT_1
Text Label 2775 4150 0    50   ~ 0
SPI2_MOSI
Wire Wire Line
	2225 4150 2775 4150
Text Label 2775 4025 0    50   ~ 0
SPI2_MISO
Wire Wire Line
	2225 4025 2775 4025
Text Label 2775 4275 0    50   ~ 0
SPI2_SCK
Wire Wire Line
	2225 4275 2775 4275
$Comp
L power:GND #PWR01
U 1 1 5EE8FBBA
P 2525 4750
F 0 "#PWR01" H 2525 4500 50  0001 C CNN
F 1 "GND" H 2530 4577 50  0000 C CNN
F 2 "" H 2525 4750 50  0001 C CNN
F 3 "" H 2525 4750 50  0001 C CNN
	1    2525 4750
	1    0    0    -1  
$EndComp
Text Notes 550  3775 0    50   ~ 0
VDD: 1.62-3.6V\nLogic: 1.2-3.6V\n20mA
Text Notes 1075 4250 2    138  ~ 0
GYRO
Wire Wire Line
	2400 3000 2750 3000
Connection ~ 2400 3000
Wire Wire Line
	2400 3100 2400 3000
Wire Wire Line
	2225 3100 2400 3100
Wire Wire Line
	2225 3000 2400 3000
Text Label 2750 3000 0    50   ~ 0
3.3V_MAG
Wire Wire Line
	2750 2550 2225 2550
Text Label 2750 2550 0    50   ~ 0
MAG_CS
Wire Wire Line
	2225 2650 2750 2650
Text Label 2750 2650 0    50   ~ 0
MAG_INT
Wire Wire Line
	2225 2450 2750 2450
Wire Wire Line
	2225 2350 2750 2350
Wire Wire Line
	2225 2250 2750 2250
Text Label 2750 2450 0    50   ~ 0
SPI1_MOSI
Text Label 2750 2350 0    50   ~ 0
SPI1_MISO
Text Label 2750 2250 0    50   ~ 0
SPI1_SCK
$Comp
L power:GND #PWR02
U 1 1 5EE9EFE7
P 2525 3200
F 0 "#PWR02" H 2525 2950 50  0001 C CNN
F 1 "GND" H 2530 3027 50  0000 C CNN
F 2 "" H 2525 3200 50  0001 C CNN
F 3 "" H 2525 3200 50  0001 C CNN
	1    2525 3200
	1    0    0    -1  
$EndComp
Text Notes 550  2450 0    50   ~ 0
VDD: 2.16-3.6V\nLogic: 1.62-3.6V\n10mA
Text Notes 975  2875 2    138  ~ 0
MAG
$Sheet
S 525  2175 1700 1150
U 5EAB13AE
F0 "MAG" 50
F1 "MAG.sch" 50
F2 "VDD" O R 2225 3100 50 
F3 "VSS" O R 2225 3200 50 
F4 "CLK" I R 2225 2250 50 
F5 "CS" I R 2225 2550 50 
F6 "MISO" O R 2225 2350 50 
F7 "INT" O R 2225 2650 50 
F8 "MOSI_SDA" I R 2225 2450 50 
F9 "VDDIO" O R 2225 3000 50 
$EndSheet
Wire Wire Line
	2775 1725 2225 1725
Text Label 2775 1725 0    50   ~ 0
3.3V_TEMPERATURE
$Comp
L power:GND #PWR05
U 1 1 5EEAE842
P 2500 1825
F 0 "#PWR05" H 2500 1575 50  0001 C CNN
F 1 "GND" H 2505 1652 50  0000 C CNN
F 2 "" H 2500 1825 50  0001 C CNN
F 3 "" H 2500 1825 50  0001 C CNN
	1    2500 1825
	1    0    0    -1  
$EndComp
Text Label 2775 1450 0    50   ~ 0
ADC1_IN10
Text Label 2775 1350 0    50   ~ 0
ADC1_IN9
Text Label 2775 1250 0    50   ~ 0
ADC1_IN8
Text Label 2775 1150 0    50   ~ 0
ADC1_IN7
Wire Wire Line
	2225 1450 2775 1450
Wire Wire Line
	2225 1350 2775 1350
Wire Wire Line
	2225 1250 2775 1250
Wire Wire Line
	2225 1150 2775 1150
Text Label 2775 1050 0    50   ~ 0
ADC1_IN4
Text Label 2775 950  0    50   ~ 0
ADC1_IN3
Text Label 2775 850  0    50   ~ 0
ADC1_IN2
Text Label 2775 750  0    50   ~ 0
ADC1_IN1
Wire Wire Line
	2225 1050 2775 1050
Wire Wire Line
	2225 950  2775 950 
Wire Wire Line
	2225 850  2775 850 
Wire Wire Line
	2225 750  2775 750 
Text Notes 550  725  0    50   ~ 0
Supply with max ADC Voltage\n
Text Notes 575  1450 0    138  ~ 0
TEMP \nSENSOR
$Sheet
S 525  625  1700 1350
U 5EAB0B79
F0 "TEMPERATURE_ADC" 50
F1 "TEMPERATURE_ADC.sch" 50
F2 "VDD" I R 2225 1725 50 
F3 "VSS" I R 2225 1825 50 
F4 "OUT1" O R 2225 750 50 
F5 "OUT2" O R 2225 850 50 
F6 "OUT3" O R 2225 950 50 
F7 "OUT4" O R 2225 1050 50 
F8 "OUT5" O R 2225 1150 50 
F9 "OUT6" O R 2225 1250 50 
F10 "OUT7" O R 2225 1350 50 
F11 "OUT8" O R 2225 1450 50 
$EndSheet
Text Notes 9275 2600 0    50   ~ 0
3.3V_STM_CORE      20mA peak, budget 50mA
Text Label 2775 4525 0    50   ~ 0
3.3V_GYRO
Wire Wire Line
	2775 4525 2400 4525
Wire Wire Line
	2400 4525 2400 4625
Wire Wire Line
	2400 4625 2225 4625
Wire Wire Line
	2225 4525 2400 4525
Connection ~ 2400 4525
Text Label 14325 1450 2    50   ~ 0
3.3V_TEMPERATURE
Text Label 14325 1350 2    50   ~ 0
3.3V_MAG
Text Label 14325 1250 2    50   ~ 0
3.3V_GYRO
Text Label 14325 1150 2    50   ~ 0
3.3V_ACS_LOGIC
Text Label 14325 1050 2    50   ~ 0
5V_ACS_COIL
Text Label 14325 950  2    50   ~ 0
5V_IRIDIUM
Text Label 14325 850  2    50   ~ 0
3.3V_GPS
Text Label 14325 750  2    50   ~ 0
3.3V_SD_CARD
Text Label 14325 1550 2    50   ~ 0
3.3V_MRAM
Text Notes 15225 2100 0    138  ~ 0
INTERBOARD \nCONNECTOR\n
Wire Wire Line
	2225 4750 2525 4750
Wire Wire Line
	2225 1825 2500 1825
Wire Wire Line
	2225 6350 2475 6350
Wire Wire Line
	2225 3200 2525 3200
Wire Wire Line
	2225 8025 2550 8025
Wire Wire Line
	2225 10200 2475 10200
Connection ~ 7800 9775
Text Notes 16450 8725 2    138  ~ 0
MRAM
$Comp
L power:GND #PWR020
U 1 1 5EECE6E3
P 14625 9000
F 0 "#PWR020" H 14625 8750 50  0001 C CNN
F 1 "GND" H 14630 8827 50  0000 C CNN
F 2 "" H 14625 9000 50  0001 C CNN
F 3 "" H 14625 9000 50  0001 C CNN
	1    14625 9000
	1    0    0    -1  
$EndComp
Wire Wire Line
	14625 9000 14875 9000
$Comp
L power:GND #PWR0101
U 1 1 62D249C8
P 14625 7775
F 0 "#PWR0101" H 14625 7525 50  0001 C CNN
F 1 "GND" H 14630 7602 50  0000 C CNN
F 2 "" H 14625 7775 50  0001 C CNN
F 3 "" H 14625 7775 50  0001 C CNN
	1    14625 7775
	1    0    0    -1  
$EndComp
Wire Wire Line
	14625 7775 14875 7775
$Sheet
S 11450 3575 1575 700 
U 602B3ABB
F0 "CAN_TRANSCEIVER" 50
F1 "CAN_TRANSCEIVER.sch" 50
F2 "PWR" I L 11450 4125 50 
F3 "GND" I L 11450 4225 50 
F4 "CAN_TX" I L 11450 4025 50 
F5 "CAN_RX" O L 11450 3925 50 
F6 "CANH" O R 13025 3925 50 
F7 "CANL" O R 13025 4025 50 
F8 "RS" O L 11450 3825 50 
$EndSheet
Wire Wire Line
	11175 4275 11175 4225
Wire Wire Line
	11175 4225 11450 4225
Text Notes 11675 4775 0    50   ~ 0
TODO: Power to CAN 
Text Label 14325 650  2    50   ~ 0
3.3V_STM_CORE
Wire Wire Line
	14325 1550 14875 1550
Wire Wire Line
	14500 3675 14875 3675
Wire Wire Line
	14325 1450 14875 1450
Wire Wire Line
	14325 1350 14875 1350
Wire Wire Line
	14325 1250 14875 1250
Wire Wire Line
	14325 1150 14875 1150
Wire Wire Line
	14325 1050 14875 1050
Wire Wire Line
	14325 950  14875 950 
Wire Wire Line
	14325 850  14875 850 
Wire Wire Line
	14325 750  14875 750 
Wire Wire Line
	14325 650  14875 650 
Wire Wire Line
	14150 1950 14875 1950
Wire Wire Line
	14250 2050 14875 2050
Wire Wire Line
	13950 2150 14875 2150
Wire Wire Line
	9500 2825 13950 2825
Wire Wire Line
	14050 2250 14875 2250
Wire Wire Line
	9500 2925 14050 2925
Wire Wire Line
	7700 2200 7625 2200
Wire Wire Line
	7700 2200 7700 1975
Wire Wire Line
	7700 1975 7625 1975
Connection ~ 7700 2200
Wire Wire Line
	7625 1750 7700 1750
Wire Wire Line
	7700 1750 7700 1975
Connection ~ 7700 1975
Connection ~ 7700 1750
Wire Wire Line
	7625 1525 7700 1525
Wire Wire Line
	7700 1525 7700 1750
Wire Wire Line
	7700 1300 7700 1525
Connection ~ 7700 1300
Wire Wire Line
	7700 1300 7625 1300
Wire Wire Line
	7625 1075 7700 1075
Wire Wire Line
	7700 1075 7700 1300
Wire Wire Line
	7625 850  7700 850 
Connection ~ 7700 1075
Wire Wire Line
	8150 1750 8150 1975
Connection ~ 8150 1975
Wire Wire Line
	8075 1750 8150 1750
Wire Wire Line
	8075 1975 8150 1975
Wire Wire Line
	8075 2200 8150 2200
Wire Wire Line
	7700 1750 7875 1750
Wire Wire Line
	7700 1975 7875 1975
Wire Wire Line
	7700 2200 7875 2200
Wire Wire Line
	7700 850  9600 850 
$Sheet
S 14875 5125 1600 1600
U 5EAB0ED7
F0 "IRIDIUM_INTERFACE" 50
F1 "IRIDIUM_INTERFACE.sch" 50
F2 "PWR" I L 14875 6450 50 
F3 "GND" I L 14875 6650 50 
F4 "ON_OFF" I L 14875 6175 50 
F5 "DF_S_TX" I L 14875 5200 50 
F6 "DF_S_RX" O L 14875 5300 50 
F7 "SIG_GND" I L 14875 6550 50 
F8 "DF_DCD" O L 14875 5775 50 
F9 "DF_DSR" O L 14875 5875 50 
F10 "DF_CTS" O L 14875 5400 50 
F11 "DF_RI" O L 14875 5975 50 
F12 "DF_RTS" I L 14875 5500 50 
F13 "DF_DTR" I L 14875 5675 50 
F14 "NET_AVAIL" O L 14875 6075 50 
F15 "SUPPLY_OUT" O L 14875 6275 50 
$EndSheet
Wire Wire Line
	14500 6550 14700 6550
Connection ~ 14700 6550
Text Notes 16450 5300 2    50   ~ 0
5.0V PWR \n2A. Softstart
Wire Wire Line
	13025 3925 14150 3925
Wire Wire Line
	14150 3925 14150 1950
Wire Wire Line
	13025 4025 14250 4025
$Sheet
S 14875 575  1600 3175
U 5EAB69E2
F0 "INTERBOARD_CONNECTOR" 50
F1 "INTERBOARD_CONNECTOR.sch" 50
F2 "PWR_STM_CORE" O L 14875 650 50 
F3 "PWR_SD_CARD" O L 14875 750 50 
F4 "PWR_GPS" O L 14875 850 50 
F5 "PWR_IRIDIUM" O L 14875 950 50 
F6 "PWR_ACS_COIL" O L 14875 1050 50 
F7 "PWR_ACS_LOGIC" O L 14875 1150 50 
F8 "PWR_GYRO" O L 14875 1250 50 
F9 "PWR_MAG" O L 14875 1350 50 
F10 "PWR_TEMPERATURE" O L 14875 1450 50 
F11 "CANH" O L 14875 1950 50 
F12 "CANL" O L 14875 2050 50 
F13 "UART_TXO" O L 14875 2150 50 
F14 "UART_RXI" O L 14875 2250 50 
F15 "GND" O L 14875 3675 50 
F16 "SPARE_8" O L 14875 3175 50 
F17 "SPARE_6" O L 14875 2975 50 
F18 "SPARE_4" O L 14875 2775 50 
F19 "SPARE_10" O L 14875 3375 50 
F20 "SPARE_2" O L 14875 2575 50 
F21 "SPARE_7" O L 14875 3075 50 
F22 "SPARE_9" O L 14875 3275 50 
F23 "SPARE_5" O L 14875 2875 50 
F24 "SPARE_3" O L 14875 2675 50 
F25 "SPARE_1" O L 14875 2475 50 
F26 "PWR_MRAM" O L 14875 1550 50 
F27 "PWR_CAN" O L 14875 1650 50 
$EndSheet
Wire Wire Line
	14250 2050 14250 4025
Wire Wire Line
	8800 2350 8900 2350
Connection ~ 8900 2350
Wire Wire Line
	8900 2350 8900 2525
Wire Wire Line
	7200 2350 7700 2350
Wire Wire Line
	7700 2200 7700 2350
Connection ~ 7700 2350
Wire Wire Line
	7700 2350 7800 2350
Wire Wire Line
	8700 2200 8700 2525
Connection ~ 8700 2200
Connection ~ 7200 1525
Connection ~ 7700 1525
Wire Wire Line
	10400 850  10400 1225
$Comp
L Device:R R2
U 1 1 615C7C05
P 9750 850
F 0 "R2" V 9543 850 50  0000 C CNN
F 1 "10m" V 9634 850 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 9680 850 50  0001 C CNN
F 3 "~" H 9750 850 50  0001 C CNN
F 4 "ERJ-3LWJR010V" V 9750 850 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/panasonic-electronic-components/ERJ-3LWJR010V/P19190CT-ND/6004545" V 9750 850 50  0001 C CNN "Order"
	1    9750 850 
	0    1    1    0   
$EndComp
Text Notes 9900 825  0    79   ~ 0
VDD
Wire Wire Line
	9900 850  10400 850 
Connection ~ 7700 850 
Wire Wire Line
	7700 850  7700 1075
Wire Wire Line
	6625 1725 6625 2825
Wire Wire Line
	5300 1725 6625 1725
Wire Wire Line
	13950 2825 13950 2150
Wire Wire Line
	14050 2925 14050 2250
$Sheet
S 525  3525 1700 1325
U 5EAB163E
F0 "GYRO" 50
F1 "GYRO.sch" 50
F2 "VDD" O R 2225 4625 50 
F3 "CS" O R 2225 3900 50 
F4 "MISO" O R 2225 4025 50 
F5 "MOSI" O R 2225 4150 50 
F6 "CLK" O R 2225 4275 50 
F7 "VDD_IO" O R 2225 4525 50 
F8 "INT2" O R 2225 3775 50 
F9 "INT1" O R 2225 3650 50 
F10 "GND" O R 2225 4750 50 
$EndSheet
Text Notes 13000 3750 2    50   ~ 0
3-3.3V Power\n20mA
Wire Wire Line
	14875 6275 14325 6275
NoConn ~ 14325 6275
Text Label 6550 4125 2    50   ~ 0
RCC_OSC_IN
Text Label 6550 4225 2    50   ~ 0
RCC_OSC_OUT
Text Label 10050 7625 0    50   ~ 0
RCC_OSC32_IN
Text Label 10050 7725 0    50   ~ 0
RCC_OSC32_OUT
$Comp
L Device:C_Small C3
U 1 1 6511C347
P 7975 1750
F 0 "C3" V 7875 1825 50  0000 C CNN
F 1 "4.7uF" V 7850 1625 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7975 1750 50  0001 C CNN
F 3 "~" H 7975 1750 50  0001 C CNN
F 4 "C1608X7S1A475K080AC" V 7975 1750 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/tdk-corporation/C1608X7S1A475K080AC/445-14258-1-ND/3955924" V 7975 1750 50  0001 C CNN "Order"
F 6 "10V" V 7925 1600 50  0000 C CNN "Voltage"
	1    7975 1750
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C13
U 1 1 6511CD12
P 8475 1975
F 0 "C13" V 8375 2075 50  0000 C CNN
F 1 "0.01uF" V 8350 1850 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 8475 1975 50  0001 C CNN
F 3 "~" H 8475 1975 50  0001 C CNN
F 4 "CL10B103KB8NCNC" V 8475 1975 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B103KB8NCNC/1276-1921-1-ND/3890007" V 8475 1975 50  0001 C CNN "Order"
F 6 "50V" V 8425 1800 50  0000 C CNN "Voltage"
	1    8475 1975
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C14
U 1 1 651584BC
P 8475 2200
F 0 "C14" V 8375 2275 50  0000 C CNN
F 1 "1uF" V 8350 2075 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 8475 2200 50  0001 C CNN
F 3 "~" H 8475 2200 50  0001 C CNN
F 4 "CL10B105KP8NFNC" V 8475 2200 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B105KP8NFNC/1276-1945-1-ND/3890031" V 8475 2200 50  0001 C CNN "Order"
F 6 "10V" V 8425 2075 50  0000 C CNN "Voltage"
	1    8475 2200
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C1
U 1 1 6515A57C
P 5775 3175
F 0 "C1" H 5700 3275 50  0000 C CNN
F 1 "0.01uF" H 5625 3100 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 5775 3175 50  0001 C CNN
F 3 "~" H 5775 3175 50  0001 C CNN
F 4 "CL10B103KB8NCNC" V 5775 3175 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B103KB8NCNC/1276-1921-1-ND/3890007" V 5775 3175 50  0001 C CNN "Order"
F 6 "50V" H 5625 3175 50  0000 C CNN "Voltage"
	1    5775 3175
	-1   0    0    1   
$EndComp
$Comp
L Device:C_Small C2
U 1 1 6515AD9F
P 6125 3175
F 0 "C2" H 6025 3275 50  0000 C CNN
F 1 "0.01uF" H 5950 3100 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 6125 3175 50  0001 C CNN
F 3 "~" H 6125 3175 50  0001 C CNN
F 4 "CL10B103KB8NCNC" V 6125 3175 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B103KB8NCNC/1276-1921-1-ND/3890007" V 6125 3175 50  0001 C CNN "Order"
F 6 "50V" H 5975 3175 50  0000 C CNN "Voltage"
	1    6125 3175
	-1   0    0    1   
$EndComp
$Comp
L Device:R R3
U 1 1 6515B55E
P 9750 1225
F 0 "R3" V 9543 1225 50  0000 C CNN
F 1 "10m" V 9634 1225 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 9680 1225 50  0001 C CNN
F 3 "~" H 9750 1225 50  0001 C CNN
F 4 "ERJ-3LWJR010V" V 9750 1225 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/panasonic-electronic-components/ERJ-3LWJR010V/P19190CT-ND/6004545" V 9750 1225 50  0001 C CNN "Order"
	1    9750 1225
	0    1    1    0   
$EndComp
$Comp
L Device:R R4
U 1 1 6515B750
P 9750 1550
F 0 "R4" V 9543 1550 50  0000 C CNN
F 1 "10m" V 9634 1550 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 9680 1550 50  0001 C CNN
F 3 "~" H 9750 1550 50  0001 C CNN
F 4 "ERJ-3LWJR010V" V 9750 1550 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/panasonic-electronic-components/ERJ-3LWJR010V/P19190CT-ND/6004545" V 9750 1550 50  0001 C CNN "Order"
	1    9750 1550
	0    1    1    0   
$EndComp
Wire Wire Line
	7200 2200 7425 2200
$Comp
L Device:C_Small C4
U 1 1 6517E621
P 7525 1075
F 0 "C4" V 7425 1150 50  0000 C CNN
F 1 "0.1uF" V 7400 950 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 1075 50  0001 C CNN
F 3 "~" H 7525 1075 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 1075 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 1075 50  0001 C CNN "Order"
F 6 "25V" V 7475 925 50  0000 C CNN "Voltage"
	1    7525 1075
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C5
U 1 1 6517E77C
P 7525 1300
F 0 "C5" V 7425 1375 50  0000 C CNN
F 1 "0.1uF" V 7400 1175 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 1300 50  0001 C CNN
F 3 "~" H 7525 1300 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 1300 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 1300 50  0001 C CNN "Order"
F 6 "25V" V 7475 1150 50  0000 C CNN "Voltage"
	1    7525 1300
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C7
U 1 1 6517F47B
P 7525 1525
F 0 "C7" V 7425 1600 50  0000 C CNN
F 1 "0.1uF" V 7400 1400 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 1525 50  0001 C CNN
F 3 "~" H 7525 1525 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 1525 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 1525 50  0001 C CNN "Order"
F 6 "25V" V 7475 1375 50  0000 C CNN "Voltage"
	1    7525 1525
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C8
U 1 1 6517F484
P 7525 1750
F 0 "C8" V 7425 1825 50  0000 C CNN
F 1 "0.1uF" V 7400 1625 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 1750 50  0001 C CNN
F 3 "~" H 7525 1750 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 1750 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 1750 50  0001 C CNN "Order"
F 6 "25V" V 7475 1600 50  0000 C CNN "Voltage"
	1    7525 1750
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C9
U 1 1 651BAB76
P 7525 1975
F 0 "C9" V 7425 2050 50  0000 C CNN
F 1 "0.1uF" V 7400 1850 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 1975 50  0001 C CNN
F 3 "~" H 7525 1975 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 1975 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 1975 50  0001 C CNN "Order"
F 6 "25V" V 7475 1825 50  0000 C CNN "Voltage"
	1    7525 1975
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C10
U 1 1 651BAB7F
P 7525 2200
F 0 "C10" V 7425 2275 50  0000 C CNN
F 1 "0.1uF" V 7400 2075 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7525 2200 50  0001 C CNN
F 3 "~" H 7525 2200 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7525 2200 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7525 2200 50  0001 C CNN "Order"
F 6 "25V" V 7475 2050 50  0000 C CNN "Voltage"
	1    7525 2200
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C11
U 1 1 651F5EAB
P 7975 1975
F 0 "C11" V 7875 2050 50  0000 C CNN
F 1 "0.1uF" V 7850 1850 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7975 1975 50  0001 C CNN
F 3 "~" H 7975 1975 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7975 1975 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7975 1975 50  0001 C CNN "Order"
F 6 "25V" V 7925 1825 50  0000 C CNN "Voltage"
	1    7975 1975
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C12
U 1 1 651F6106
P 7975 2200
F 0 "C12" V 7875 2275 50  0000 C CNN
F 1 "0.1uF" V 7850 2075 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7975 2200 50  0001 C CNN
F 3 "~" H 7975 2200 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 7975 2200 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 7975 2200 50  0001 C CNN "Order"
F 6 "25V" V 7925 2050 50  0000 C CNN "Voltage"
	1    7975 2200
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C16
U 1 1 651F6327
P 9225 1975
F 0 "C16" V 9125 2050 50  0000 C CNN
F 1 "0.1uF" V 9100 1850 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9225 1975 50  0001 C CNN
F 3 "~" H 9225 1975 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 9225 1975 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 9225 1975 50  0001 C CNN "Order"
F 6 "25V" V 9175 1825 50  0000 C CNN "Voltage"
	1    9225 1975
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C17
U 1 1 651F65EC
P 9225 2200
F 0 "C17" V 9125 2275 50  0000 C CNN
F 1 "0.1uF" V 9100 2075 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9225 2200 50  0001 C CNN
F 3 "~" H 9225 2200 50  0001 C CNN
F 4 "CL10B104KA8NNNC" V 9225 2200 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics/CL10B104KA8NNNC/1276-1006-1-ND/3889092" V 9225 2200 50  0001 C CNN "Order"
F 6 "25V" V 9175 2050 50  0000 C CNN "Voltage"
	1    9225 2200
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C15
U 1 1 651FB3D9
P 9225 1750
F 0 "C15" V 9125 1825 50  0000 C CNN
F 1 "4.7uF" V 9100 1625 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9225 1750 50  0001 C CNN
F 3 "~" H 9225 1750 50  0001 C CNN
F 4 "C1608X7S1A475K080AC" V 9225 1750 50  0001 C CNN "Mfr. #"
F 5 "https://www.digikey.com/product-detail/en/tdk-corporation/C1608X7S1A475K080AC/445-14258-1-ND/3955924" V 9225 1750 50  0001 C CNN "Order"
F 6 "10V" V 9175 1600 50  0000 C CNN "Voltage"
	1    9225 1750
	0    1    1    0   
$EndComp
Wire Wire Line
	8900 1550 8900 1750
Wire Wire Line
	10400 1550 10400 1225
Wire Wire Line
	10975 3825 11450 3825
Wire Wire Line
	10975 3625 10975 3825
Wire Wire Line
	9500 3625 10975 3625
Text Label 10050 3625 0    50   ~ 0
CAN_RS
Text Label 14325 1650 2    50   ~ 0
3.3V_CAN
Wire Wire Line
	14325 1650 14875 1650
Text Label 11050 4125 2    50   ~ 0
3.3V_CAN
Wire Wire Line
	11050 4125 11450 4125
$EndSCHEMATC
